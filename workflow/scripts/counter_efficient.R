...
## get unique qnames:
d.qnames <- tibble("qname"=bam_1[[1]]$qname)
...
results <- lapply(n, function(i){
  ...
  d.bam_1_sub <- sub1 %>% as_tibble() %>% mutate_at("rname", as_character)
  d.bam_2_sub <- sub2 %>% as_tibble() %>% mutate_at("rname", as_character)
  ...
  d.bam_sub <- inner_join(d.bam_1_sub %>% select (qname, "rname_1" = rname),
                         d.bam_2_sub %>% select (qname, "rname_2" = rname),
                         by = "qname")

  ## check whether each pairing is correct
  d.bam_sub <- d.bam_sub %>% mutate("paired" = rname_1 == rname_2)
  ...
  qname2anypaired <- sapply (split (d.bam_sub$paired, f = d.bam_sub$qname), any)
  d.bam_sub <- d.bam_sub %>% left_join (tibble ("qname" = names (qname2anypaired),
  ...
  qname2n <- table (d.bam_sub$qname)
  d.bam_sub <- d.bam_sub %>% left_join (tibble ("qname" = names (qname2n),
  ...
  n_paired <- d.bam_sub %>% filter (paired) %>% summarize (sum (weight)) %>% collect() %>% _[[1]]
  n <- d.bam_sub %>% summarize (sum (weight)) %>% collect() %>% _[[1]]
  d.paired_sub <- tibble("n_correctly_paired"=n_paired, "n_total"=n)

  ## compute counts for correctly paired reads
  d.bam_sub <- d.bam_sub %>% filter (paired)

  ## return a list of the d.bam and d.paired for this subset of the BAM file
  return(list(bam=d.bam_sub, paired=d.paired_sub))
...
d.paired_all <- d.paired %>% summarize(n_paired=sum(n_correctly_paired), n_total=sum(n_total))
d.paired_all <- d.paired_all %>% mutate("pct_paired"=round(100*n_paired/n_total,3))

message (paste0 ("   ", d.paired_all$n_paired, " / ", d.paired_all$n_total,
                 " (", d.paired_all$pct_paired, "%) correctly paired reads\n"))

## check that all rnames are correctly paired
stopifnot (all (d.bam$rname_1 == d.bam$rname_2))
...
## store counts
d.counts <- d.counts %>% left_join (d_bam, by = "id")
rm (d.bam)

## some pgRNAs may have no counts; replace those NAs with 0
d.counts$counts[is_na (d.counts$counts)] = 0

## rename counts column
new_counts <- paste0("counts_", sample)
d.counts <- d.counts %>% dplyr::rename(!!new_counts := counts)
...
write_tsv(d.counts, counts_filepath)
...
# d.bam_1 = readRDS("test1_bam") %>% as_tibble()
# d.bam_2 = readRDS("test2_bam") %>% as_tibble()

### Within the lapply:
## for converting BAMs to tibbles we could use an all-purpose conversion method as follows:
##   d.bam_1 = do_call ("data_frame", bam_1) %>% as_tibble()
##   d.bam_2 = do_call ("data_frame", bam_2) %>% as_tibble()
## however, it's much faster to manually construct the tibbles
##   d.bam_1_sub = tibble("qname" = d.bam_1_sub[[1]]$qname, "rname" = d.bam_1_sub[[1]]$rname)
##   d.bam_1_sub <- d.bam_1_sub %>% mutate_at("rname", as_character)
##   d.bam_1 = tibble ("qname" = bam_1[[1]]$qname,
